...
本扩展**自行启动并管理 `dsh web` 服务**，将完整的 Web GUI——聊天、会话、工具调用、轨迹、审批、设置——以**完整编辑器标签页**的形式打开（类似 Claude Code）：点击活动栏的 DSH（DeepSeek 鲸鱼）图标，聊天作为编辑器标签页占满编辑器区打开；侧边栏不显示任何插件内容。
...
活动栏的 **DSH** 鲸鱼图标会把聊天作为编辑器标签页打开（侧边栏瞬间闪合，无插件内容）；状态栏显示服务状态，点击可打开聊天。
