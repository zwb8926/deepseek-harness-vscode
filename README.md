...
在侧边栏点一个会话，编辑器标签页会自动跟随切换（若编辑区未打开，会自动打开编辑区标签页）；两条面板共享同一会话数据。
...
- **接管服务死亡自动接管**：接管的外部 `dsh web`（例如终端里 `npx dsh web`）被停止后，扩展会在数秒内检测到并**自动启动自己的服务接管**（受 `dsh.autoRestart` 控制，有重启次数上限；若外部服务在重启前已恢复则直接重新接管）。重启预算耗尽后转为等待外部服务回归并自动重新接管。也可以随时点「启动服务」手动接管
- **不自动新建会话**：新建会话在侧边栏的 GUI 内完成，扩展只打开已有的。服务启动后扩展会把当前 VS Code 项目自动注册为 dsh workspace（`workspace.create`，幂等、不建会话），侧栏的 workspace 分组立即可见该项目
